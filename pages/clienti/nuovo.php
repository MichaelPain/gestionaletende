...
if (is_post()) {
    if (!csrf_check($_POST['csrf'] ?? '')) {
        ...
        if ($data['nome'] === '') {
            ...
            $stmt = $pdo->prepare("INSERT INTO clienti (tipo, nome, partita_iva, codice_fiscale, indirizzo, telefono, email) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $data['tipo'], $data['nome'], $data['partita_iva'], $data['codice_fiscale'],
                $data['indirizzo'], $data['telefono'], $data['email']
            ]);

            // audit
            $u = current_user();
            $log = $pdo->prepare("INSERT INTO audit_log (utente_id, azione, data_ora) VALUES (?, ?, NOW())");
            $log->execute([$u['id'], 'CREA_CLIENTE:' . $data['nome']]);
...
<main class="container">
    ...
    <form method="post">
        ...
        <div class="form-group"><label>Partita IVA</label><input name="partita_iva"></div>
        <div class="form-group"><label>Codice Fiscale</label><input name="codice_fiscale"></div>
        <div class="form-group"><label>Indirizzo</label><input name="indirizzo"></div>
